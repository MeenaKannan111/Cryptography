...
def process_input(data):
    # HEX input (32 chars → 16 bytes)
    if len(data) == 32 and all(c in "0123456789abcdefABCDEF" for c in data):
        return [int(data[i:i+2], 16) for i in range(0, 32, 2)]
    # TEXT input
    if len(data) < 16:
        data = data.ljust(16)   # pad with spaces
    return [ord(c) for c in data[:16]]
# ---------------- MAIN ----------------
def main():
    print("AES-128 Encryption")
    text = input("Enter plaintext: ")
    key = input("Enter key: ")
    plaintext = process_input(text)
    key = process_input(key)
